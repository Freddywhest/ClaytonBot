...
`
  ...
  async #run_tasks(tgClients) {
    ...
    if (settings.MAX_CONCURRENT_ACCOUNT > 2) {
      console.log("\n");
      logger.warning(
        `<ye>[clayton]</ye> | MAX_CONCURRENT_ACCOUNT greater than 2 may cause 403 errors while running the bot\n`
      );
    ...
    const tasks = tgClients.map(async (tgClient, index) => {
      return limit(async () => {
        ...
        try {
          ...
          logger.info(
            `<ye>[clayton]</ye> | ${tgClient.session_name} | Sleeping ${sleeping} seconds before starting the bot`
          );
    ...
    logger.info(
      `<ye>[clayton]</ye> | All threads completed | Sleeping ${sleep_thread} seconds before rerunning threads again`
    );
    ...
    const tasks = queries?.map(async ([query_name, query_id], index) => {
      return limit(async () => {
        ...
        try {
          ...
          logger.info(
            `<ye>[clayton]</ye> | ${query_name} | Sleeping ${sleeping} seconds before starting the bot`
          );
          await sleep(sleeping);
